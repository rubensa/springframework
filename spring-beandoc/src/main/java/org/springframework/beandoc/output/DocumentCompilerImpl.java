...
public class DocumentCompilerImpl implements DocumentCompiler {
	...
	public void compile(Document[] contextDocuments, File outputDir) {
		...
		String consolidatedImage = contextDocuments[0].getRootElement().getAttributeValue(
		...
		if (dotExe != null && new File(dotExe).isFile())
			for (int i = 0; i < dotFileList.size(); i++) {
				...
				/*
				 * insert map into -graph.html file. If the source is not in the
				 * root of the outputDir, we need to flatten the links in the
				 * map file generated by dot.
				 */
			...
			logger.warn("GraphViz 'dot' executable not set or couldn't be found.  No graphs generated.");
	...
	private File runDot(File dotFile, String outputType, String fileExt) {
		...
		try {
			...
			logger.warn("Problem attempting to create [" + outputFile + "] from dot file [" + dotFileName + "]; "
					+ ioe.getMessage());
		}
	...
	private void plugMap(File mapFile, boolean isInRootDir) {
		...
		File graphFile = new File(StringUtils.replace(mapFile.getAbsolutePath(), ".map", filenameStrategy
				.getFileName("")));
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				// if the file is not in the root, we have to amend the map
				// links
				// generated by dot. This is messy and slightly unpredictable
				// but
				// the alternative is parsing the whole file into a DOM
			...
			logger.warn("Unable to find either [" + graphFile.getAbsolutePath() + "] or [" + mapFile.getAbsolutePath()
					+ "] - graphing output probably not configured.");

		}
		catch (IOException e) {
			logger.warn("Unable to generate documentation from [" + graphFile.getAbsolutePath() + "] and ["
