...
/**
 * Base implementation of the <code>Transformer</code> interface that uses
 ...
 * {@link #getOutputForDocument} and a stylesheet URI through the
 * {@link #setTemplateName} method.
 * 
...
public class XslTransformer implements Transformer {
	...
	/**
	 * loads and compiles the stylesheets using specified values. Both
	 * parameters are interpreted as standard Spring <code>Resource</code>s
	...
	/**
	 * Implements the <code>Transformer</code> API and sanity checks some
	 * configuration aspects before beginning the transformation workflow. The
	 * <code>initTransform</code> method is first called, returning on any
	 * thrown Exception. After successful initialisation and XSL processing, the
	 * <code>postTransform()</code> method is called to perform cleanup or
	 * additional tasks.
	 * 
	 * @see #initTransform
	 */
	...
	/**
	 * Perform any initialization or one-off tasks prior to the actual
	 * transformation of the context documents with the configured stylesheet.
	...
	/**
	 * Creates an output file in the specified location for each supplied
	 * document re-using the same compiled stylesheet for each. Subclasses can
	 * override this behaviour if, for example, they don't want to have each
	 * document handled in sequence. In this case, the subclass can still make
	 * use of the <code>doXslTransform</code> method aas a library function.
	 * 
	 * @param contextDocuments the array of DOM trees about to be transformed
	 * @param outputDirectory the file handle for the output directory
	 */
	...
	/**
	 * perform the actual tree transformation to the output directory for the
	 * given Document
	 * 
	...
	protected void doXslTransform(Document doc, File outputDir) {
		...
		try {
			...
			Result result = new StreamResult(new BufferedOutputStream(new FileOutputStream(outputFile)));
			...
			trans.setErrorListener(new ErrorListener(){
				public void warning(TransformerException exception) throws TransformerException {
					logger.warn("Warning during transformation", exception);
				}

				public void error(TransformerException exception) throws TransformerException {
					logger.error("Error during transformation", exception);
				}

				public void fatalError(TransformerException exception) throws TransformerException {
					logger.fatal("FatalError during transformation", exception);
				}
			});
	 ...
	 * @return a Map of stylesheet parameter names and their corresponding
	 * values. The parameter names must be defined in the stylesheet itself.
	 */
	...
	/**
	 * Return the name of the output file (relative to the configured output
	 * directory) that this transformer will use to generate output to. Default
	 * impl. uses the filenameStrategy.
	 * 
	 * @param inputFileName the original file name (not including path) of the
	 * context file
	...
	protected String getOutputForDocument(String inputFileName) {
		...
		logger.debug("Returning output file name [" + output + "] for input file name [" + inputFileName + "]");
		return output;
	}

	/**
	 * Perform any finalization or one-off tasks after the actual transformation
	 * of the context documents with the configured stylesheet. Default
	...
	/**
	 * The String value representing the resource pointing to an XSL stylesheet
	 * used in generating documentation output.
	 * 
	...
	/**
	 * loads and compile the stylesheet used in generating document output. If
	 * the supplied template name cannot be compiled to a binary template
	 * representation, a BeanDocException will be thrown here.
	 * 
	 * @param templateName a standard Spring <code>Resource</code> that can
	...
	public void setTemplateName(String templateName) throws InvalidTransformerException {
		...
		try {
			...
			this.templates = this.transformerFactory.newTemplates(new StreamSource(res.getInputStream(), systemId));
		}
		catch (Exception ex) {
			throw new InvalidTransformerException("Can't load stylesheet [" + templateName + "]" + ex.getMessage(), ex);
		}
	}

	/**
	 * @return the Map of static XSL parameters that may or may not be returned
	 * as part of the getParameters(Document) method
	 */
	...
	/**
	 * Provide a Map of XSL parameter values for this class that can be used to
	 * make up all or part of the Map of actual parameters used in the
	 * stylesheet.
	 * 
	 * @param staticParameters a Map of parameters that will probably be used as
	 * part of a Map returned from the getParameters(Document) method
	 */
