...
public class BeanDocUtils {
	...
	/**
	 * Returns a new <code>Map</code> containing only those entries from
	 * <code>map</code> whose key starts with <code>prefix</code>. Prefixes
	 * are removed from the keys in the returned <code>Map</code>.
	 * 
	 * @param map the original <code>Map</code> to be filtered; must not be
	 * <code>null</code>
	 * @param prefix prefix by which entries are filtered; may be empty of
	 * <code>null</code>
	...
	/**
	 * Returns a new <code>Map</code> containing only those entries from
	 * <code>map</code> whose key starts with <code>prefix</code>.
	 * 
	 * @param map the original <code>Map</code> to be filtered; must not be
	 * <code>null</code>
	 * @param prefix prefix by which entries are filtered; may be empty of
	 * <code>null</code>
	 * @param removePrefix should prefixes be removed from entries in the
	 * returned <code>Map</code>?
	 * @return a new <code>Map</code> with filtered entries
	 */
	public static Map filterByPrefix(final Map map, final String prefix, final boolean removePrefix) {
		...
		for (Iterator it = map.entrySet().iterator(); it.hasNext();) {
			try {
				...
				if (name.startsWith(prefix)) {
					filteredMap.put(removePrefix ? name.substring(prefix.length()) : name, entry.getValue());
				}
			}
			catch (Exception e) {
	 ...
	 * @return an array of <code>Resource</code>'s resolved from the input
	 * names
	 */
	public static Resource[] getResources(String[] inputFileNames) throws IOException {
		// resolve resources assuming Files as the default (rather than
		// classpath resources)
		...
		Resource[] inputFiles = (Resource[]) allResources.toArray(new Resource[allResources.size()]);

		return inputFiles;
	}

	/**
	 * For the given array of Resources, return an equivalent length array of
	 * filenames that uniquely identify each file-system resource from the
	 * shortest possible common root.
	 * <p>
	 ...
	 * 
	 * @param inputFiles an array of Resources which must be resolvable as Files
	...
	public static String[] normaliseFileNames(Resource[] inputFiles) {
		...
		for (int i = 0; i < numFiles; i++)
			try {
				tokenList.add(inputFiles[i].getFile().getCanonicalPath().split(FILE_SEP_REGEX));
			}
			catch (IOException e) {
	...
	/**
	 * Return a String denoting a relative path marker for the input file name
	 * (String). For example, an input of "foo/bar/baz.html" has a relative
	 * marker of "../../" denoting the two path components.
	 * 
	 * @param input
	...
	/**
	 * Returns a <code>List</code> of <code>File</code> objects based on
	 * criteria specified. The functionality is equivalent to
	 * <code>File.listFiles()</code> but with the added benefit of recursing
	 * all subdirectories from the root directory supplied.
	 * 
	 * @param rootDir A <code>File</code> object which indicates the root
	 * location to begin searching matching files. If not an existing directory
	 * on disk, an <code>IllegalArgumentException</code> will be thrown.
	 * @param filter A <code>FileFilter</code> that is used to specify which
	 * <code>File</code>'s should be returned.
	 * @return A <code>List</code> of <code>File</code> objects that match
	 * the criteria specified in the <code>FileFilter</code> parameter
	 */
