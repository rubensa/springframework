	  		...
	  		<body>
	    		...
	    		<p>
	    			<a href="{beans/@beandocConsolidatedImage}" target="_secondary" title="View graph for this context">
	    				<img src="{beans/@beandocConsolidatedImage}" height="100" width="180" alt="View graph for this context"/> View graph for the consolidated context
	    			</a>
    			</p>
    			</xsl:if>
    			...
    			<table id="fileListTable" summary="List of individual context files and their descriptions that 
    				...
    				<tbody>
    				<xsl:for-each select="beans/description">
    					<tr>
    						<td style="width:30%">
    							<strong><xsl:value-of select="@beandocFileName"/></strong><br/>
    							<xsl:if test="@beandocGraphName">
    							(<a href="{@beandocGraphName}" target="_secondary" title="View Graph">view graph</a>)
    							</xsl:if>
    						</td>
