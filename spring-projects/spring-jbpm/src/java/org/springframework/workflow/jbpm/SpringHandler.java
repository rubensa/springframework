 ...
 *      &lt;factoryName>myFactoryInstance&lt;factoryName>
 *	&lt;/action>
 * </pre>
 * 
 * where beanName represents a jBPM actionHandler defined inside Spring container using
 * it's capabilities (IoC, AOP, etc). The optional factoryName parameter is used to specify
 * the key under which the bean factory can be found; if there is only one JbpmFactoryBean or
 * JbpmFactoryLocator inside the classloader, factoryName can be skipped. 
 * 
 * @author Costin Leau
 *
 */
public class SpringHandler implements ActionHandler, AssignmentHandler, DecisionHandler {

	private static final Log logger = LogFactory.getLog(SpringHandler.class);
