...
import org.springframework.webflow.config.support.FlowScopeExpression;
import org.springframework.webflow.config.support.ParameterizableFlowAttributeMapper;
import org.springframework.webflow.config.support.TransitionCriteriaChain;
...
/**
 * Flow builder that builds flows as defined in an XML document object model (DOM)
 * element. The element is supposed to be read from an XML file that uses the
 * following doctype:
 * 
 * <pre>
 ...
 * 
 * <p>
 * This builder will setup a flow-local bean factory for the flow being constructed.
 * That flow-local bean factory will be populated with the XML bean definition file
 * referenced using the "resource" element. The flow-local bean factory will use
 * the bean factory defing this flow builder as a parent. As such, the flow can access
 * artifacts in either its flow-local bean factory, or in the parent bean factory
 * hierarchy, e.g. the bean factory of the dispatcher.
 * 
 * @author Erwin Vervaet
 */
public class DomFlowBuilder extends BaseFlowBuilder {
	...
	/**
	 * The root element from which to obtain flow definition information.
	 */
	 ...
	 */
	protected FlowArtifactFactory flowArtifactFactory;
	
	/**
	 * Default constructor for bean style usage.
	 */
	public DomFlowBuilder() {
	}
	
	/**
	 * Create a new DOM flow builder as defined by given bean factory.
	 * @param beanFactory the bean factory defining this flow builder
	 */
	public DomFlowBuilder(BeanFactory beanFactory) {
		super(beanFactory);
	}
	 ...
	 */
	public DomFlowBuilder(BeanFactory beanFactory, Element documentElement) {
		super(beanFactory);
		this.documentElement = documentElement;
	}
	
	/**
	 * Returns the DOM element that's parsed by this builder.
	 */
	...
	/**
	 * Set the document element parsed by this builder.
	 */
	...
	/**
	 * Returns the helper used to access flow definition artifacts in
	 * the bean factory.
	 */
	protected FlowArtifactFactory getFlowArtifactFactory() {
		Assert.notNull(flowArtifactFactory, "Flow construction not properly initialized -- Call init()!");
	...
	public void dispose() {
		setFlow(null);
		flowArtifactFactory = null;
	}

	/**
	 * Initialize a flow artifact factory helper to access the flow local
	 * bean factory. Sets the {@link #flowArtifactFactory} member variable.
	 */
	protected void initFlowArtifactFactory() {
		List importElements = DomUtils.getChildElementsByTagName(getDocumentElement(), IMPORT_ELEMENT);
		...
		//create a flow local bean factory, as a child of the bean factory defining this flow builder
		DefaultListableBeanFactory flowLocalBeanFactory = new DefaultListableBeanFactory(getBeanFactory());
		new XmlBeanDefinitionReader(flowLocalBeanFactory).loadBeanDefinitions(resources);
		flowArtifactFactory = new FlowArtifactFactory(flowLocalBeanFactory);
	}
	 ...
	 */
	protected void addInlineFlowDefinitions() {
		List inlineFlowElements = DomUtils.getChildElementsByTagName(getDocumentElement(), INLINE_FLOW_ELEMENT);
		...
		for (int i = 0; i < inlineFlowElements.size(); i++) {
			...
			Element inlineFlowDefElement = (Element)inlineFlowElement.getElementsByTagName(FLOW_ATTRIBUTE).item(0);
			
			//register a bean definition for this inline flow definition
			...
			getFlowArtifactFactory().getBeanDefinitionRegistry()
					.registerBeanDefinition(builderId(inlineFlowId), inlineFlowBuilderBeanDef);
			getFlowArtifactFactory().getBeanDefinitionRegistry()
					.registerBeanDefinition(inlineFlowId, inlineFlowFactoryBeanDef);
		}
	}
	 ...
	 * Will not parse all state definitions for the flow!
	 */
	protected Flow parseFlow(String id, Map properties) {
		Assert.state(FLOW_ELEMENT.equals(getDocumentElement().getTagName()));
		Flow flow = (Flow)getFlowArtifactFactory().getOrCreateArtifact(
				getDocumentElement().getAttribute(BEAN_ATTRIBUTE),
				getDocumentElement().getAttribute(ID_ATTRIBUTE),
				Flow.class);
		flow.setId(id);
		Map flowProperties = parseProperties(getDocumentElement());
		if (properties != null) {
	...
	protected void addStateDefinitions() {
		String startStateId = getDocumentElement().getAttribute(START_STATE_ATTRIBUTE);
		...
		getFlow().setStartState(startStateId);
	}

	/**
	 * Parse all state entry and exit actions defined in given element and add them to
	 * given state.
	 */
	 ...
	 * the flow.
	 */
	protected ActionState parseActionState(Element element) {
		ActionState actionState = (ActionState)getFlowArtifactFactory().getOrCreateArtifact(
				element.getAttribute(BEAN_ATTRIBUTE),
				element.getAttribute(ID_ATTRIBUTE),
				ActionState.class);
		actionState.setId(element.getAttribute(ID_ATTRIBUTE));
		actionState.setFlow(getFlow());
	...
	/**
	 * Parse given view state definition and add a corresponding state to
	 * the flow.
	 */
	protected ViewState parseViewState(Element element) {
		ViewState viewState = (ViewState)getFlowArtifactFactory().getOrCreateArtifact(
				element.getAttribute(BEAN_ATTRIBUTE),
				element.getAttribute(ID_ATTRIBUTE),
				ViewState.class);
		viewState.setId(element.getAttribute(ID_ATTRIBUTE));
		viewState.setFlow(getFlow());
		if (element.hasAttribute(VIEW_ATTRIBUTE)) {
			viewState.setViewSelector(
					(ViewSelector)fromStringTo(ViewSelector.class).execute(element.getAttribute(VIEW_ATTRIBUTE)));
		}
	 ...
	 * the flow.
	 */
	protected DecisionState parseDecisionState(Element element) {
		DecisionState decisionState = (DecisionState)getFlowArtifactFactory().getOrCreateArtifact(
				element.getAttribute(BEAN_ATTRIBUTE),
				element.getAttribute(ID_ATTRIBUTE),
				DecisionState.class);
		decisionState.setId(element.getAttribute(ID_ATTRIBUTE));
		decisionState.setFlow(getFlow());
	...
	/**
	 * Parse given subflow state definition and add a corresponding state to
	 * the flow.
	 */
	protected SubflowState parseSubflowState(Element element) {
		SubflowState subflowState = (SubflowState)getFlowArtifactFactory().getOrCreateArtifact(
				element.getAttribute(BEAN_ATTRIBUTE),
				element.getAttribute(ID_ATTRIBUTE),
				SubflowState.class);
	 ...
	 * flow.
	 */
	protected EndState parseEndState(Element element) {
		EndState endState = (EndState)getFlowArtifactFactory().getOrCreateArtifact(
				element.getAttribute(BEAN_ATTRIBUTE),
				element.getAttribute(ID_ATTRIBUTE),
				EndState.class);
		endState.setId(element.getAttribute(ID_ATTRIBUTE));
		endState.setFlow(getFlow());
		if (element.hasAttribute(VIEW_ATTRIBUTE)) {
			endState.setViewSelector(
					(ViewSelector)fromStringTo(ViewSelector.class).execute(element.getAttribute(VIEW_ATTRIBUTE)));
		}
	...
	protected Transition parseTransition(Element element) {
		Transition transition = (Transition)getFlowArtifactFactory().getOrCreateArtifact(
				element.getAttribute(BEAN_ATTRIBUTE),
	...
	/**
	 * Add all flow exception handlers to the flow.
	 */
