...
public class RedirectViewDescriptorCreator extends SimpleViewDescriptorCreator {
	...
	protected void parseEncodedView(String encodedView, ExpressionParser parser) {
		...
		if (array.length == 2) {
			...
			for (int i = 0; i < array.length; i++) {
				...
				Expression value = getValue((String)nameValue[1], parser);
	 ...
	 */
	protected Expression getValue(String valueString, ExpressionParser parser) {
		if (parser.isExpression(valueString)) {
			return parser.parseExpression(valueString);
		}
		else {
			return new StaticExpression(valueString);
		}
	}
	...
	public ViewDescriptor createViewDescriptor(RequestContext context) {
		...
		while (it.hasNext()) {
			...
			Expression evaluator = (Expression)entry.getValue();
			viewDescriptor.addObject(attributeName, evaluator.evaluateAgainst(context, getEvaluationContext(context)));
		}
