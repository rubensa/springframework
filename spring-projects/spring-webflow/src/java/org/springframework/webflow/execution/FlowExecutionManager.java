 ...
 * responsible for creating new flow executions as requested by the client, as
 * well as signaling events for processing by existing, paused executions (that
 * are waiting to be resumed in response to a user event).
 * <p>
 ...
 * using a flow locator. If no "_flowId" parameter is present, the default
 * top-level flow configured for this manager is used.</li>
 ...
 * extracted from the event. The event is then signaled in that state, and the
 * executing flow is resumed in that state.</li>
 * <li>If the flow execution is still active after event processing, it is
 * saved in storage. This process generates a unique flow execution id that will
 * be exposed to the caller for reference on subsequent events. The caller will
 * also be given access to the flow execution context and any data placed in
