...
/**
 * JSF phase listener that is responsible for loading the current flow execution to a
 * threadlocal, so that components may bind to it as needed.
 * 
 * Note: this class currently uses a FlowNavigationHandlerStrategy for convenience. That 
 * class may actually be broken up.
 * 
 * @since 1.0
	...
	public void afterPhase(PhaseEvent event) {
		...
		if (event.getPhaseId() == PhaseId.RESTORE_VIEW) {
			...
			if (flowExecutionId != null) {
				FlowExecution flowExecution = strategy.loadFlowExecution(jsfEvent, null);
