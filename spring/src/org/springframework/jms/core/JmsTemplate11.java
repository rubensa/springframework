 ...
 * {@link JmsTemplate#setDynamicDestinationsEnabled(boolean) setEnabledDynamicDestinations}to enable this
 * functionality.
 *
 ...
 * @author Mark Pollack
 */
public class JmsTemplate11 extends JmsTemplate {
		...
		if (this.getConverter() == null) {
			...
			send(d, new MessageCreator() {
				public Message createMessage(Session session)
				    throws JMSException {
					return getConverter().toMessage(o, session);
				}
			});
		}
	}
	...
	public void send(
		...
		if (this.getConverter() == null) {
			...
			send(d, new MessageCreator() {
				public Message createMessage(Session session)
				    throws JMSException {
					Message m = getConverter().toMessage(o, session);
